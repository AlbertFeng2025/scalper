...
**Which book?** `0101` was discovered and is used on the **SHORT book** (SHORT 32/19, morning). It also *works* on the LONG book but more weakly — see the both-books table below. The primary application is **SHORT**.

**What it is.** `0101` = loss-win-loss-win, **ending on the reversion phase** (`...01`). It directly detects *"the market is currently oscillating and just reverted."* The alternation **is** the regime confirmation that the F2 gate normally provides — which is why it works as a **single layer** (Layer 1), unlike the two-layer finalists that need an explicit F2 gate.

### Full numbers — both books (32/19, morning, clean 50-file recompute + OOS week)

| Book | In-sample fires | Win rate | **MCL** | $/fire | Net $ | OOS (week) |
|---|---|---|---|---|---|---|
| **SHORT** *(primary)* | 139 | **70.5%** | **3** | +$7.9 | +$1,100 | 15 fires, 66.7%, MCL 2, +$60 |
| LONG | 127 | 66.9% | 3 | +$4.3 | +$542 | 11 fires, 63.6%, MCL 2, +$10 |

*(Raw morning baseline: SHORT 64.6%, LONG 64.1%. BE = 62.7%.)*

> **Reading this:** On SHORT, `0101` lifts the raw 64.6% → **70.5%** (+5.9pp over baseline, +7.8pp over break-even), on **139 fires** with a **max-consecutive-loss of 3**. It held out-of-sample (66.7% on 15 fires). On LONG it lifts less (64.1% → 66.9%, barely above BE) and makes far less per trade (+$4.3 vs +$7.9/fire) — consistent with LONG being the weak book everywhere. **Use `0101` on SHORT; on LONG it is marginal.**

> **Note on earlier figures:** the original 2026-07-11 write-up cited `0101` at 169 fires / 73.4% from the 55-day set. The table above is a cleaner 50-file recomputation with correct trading-day handling; the numbers are slightly lower (139 fires / 70.5%) but the conclusion is unchanged — it holds in-sample and out-of-sample and remains the best single-layer filter. **MCL is 3, not the 2 originally noted** — plan for it.

### The ending phase is what matters, not the length

Every extension of `0101` breaks it:

| Variant | Win rate | Verdict |
|---|---|---|
| `0101` | **70.5%** (SHORT) | ✅ the sweet spot |
...
**Lesson:** longer ≠ more selective. Extending `0101` either adds a momentum tail (`...1`, `...?`) or flips the phase (`...0`) — both destroy the reversion edge. The magic is specifically the `...01` ending: the string must *end on a just-reverted-up (win) following a loss*, which is the signature of an active oscillating regime.
