...
**Date of summary:** 2026-07-28  *(supersedes 2026-07-25; new material in §0 note, §12, and updated §8–§9 + appendix)*
**Research window:** 09:30–16:00 New York time (RTH). Core research = 21 clean days (Oct 2025 → Jul 2026, calibrated Python slicer). 2026-07-28 addendum = 18 days re-run on an independent Renko reconstruction + live-log slippage review.

---

> **⚠ 2026-07-28 headline changes (read §12 for detail):**
> - **New preferred SHORT filter: `F1=10 / F2=0100`.** On an independent 18-day reconstruction it beat the old `10/010` **out-of-sample** (36.1% vs 28.8% on 13 unseen days) and trimmed the worst loss streak 7 → **5**. It is the new lead candidate.
> - **`10/010` did NOT hold on the reconstruction** (30.6% all / 28.8% OOS, below breakeven). This may be a reconstruction difference vs the calibrated slicer (which gave 39.3%) rather than a true regression — **the discrepancy is itself a calibration flag** (§8.1 is now doubly important).
> - **`1010` was an overfit mirage:** 54% on the 5-day pick set → **21% out-of-sample.** Rejected. (Lesson: sweep winners must be OOS-confirmed.)
> - **Slippage is real but conditional:** market-entry fills distort the 10/20 geometry only under a *fast spike + lagged fill*; ordinary busy-hour fills (incl. the 09:30 open) were clean (<2 pt). SHORT fills tilt adverse. **Limit entry fixes it but is only safe with NO qty rule** (§12.3).
> - **The "skip early losses then size up" qty idea is mechanically impossible** as coded (§12.4) and IID-doomed regardless.
...
**Bottom line (updated 07-28):** the SHORT-book reversal edge is real but **thin and cost-sensitive**. Lead candidates are **`1000` (calibrated slicer)** and **`10/0100` (07-28 reconstruction, OOS-survived, MCL 5)**; **retire `10/010`** (it did not hold on the reconstruction) and **do not use `1010`** (overfit). Trade flat — **no qty ratchet**. Gross edge ≈ **+0.8 pt/trade OOS**, so **the decisive open test is now net-of-costs** (commission + the measured adverse SHORT slippage), alongside the still-unresolved **live-`rawString` calibration**.
...
*End of summary — updated 2026-07-28 (core research 2026-07-25).*
