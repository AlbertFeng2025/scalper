...
// Scalper_Renko_TransitLONGrepeat_Layer2  v1
//
// ============================================================================
// WHAT THIS IS
// ============================================================================
// A Renko-based Layer-2 meta-labeling strategy for LONG on MNQ.
// (Direction mirror of Scalper_Renko_TransitSHORTrepeat_Layer2 — same pipeline, flipped
//  encoding and flipped bracket.)
//
// RAW STRING SOURCE:
...
//     Close rose = up bar  = GREEN = bit '1' (win  for LONG)
//     Close fell = down bar = RED  = bit '0' (loss for LONG)
//
//   Target = 1 = we chase '1's = we want GREEN bars = bullish reversion.
//   (This is the ONLY encoding difference from the SHORT book: there red=1; here
//    green=1. Because the encoding flips, the SAME F1="1000"/F2="000" patterns
//    describe the mirror setup automatically — see below.)
//
// ****************************************************************************
...
//
// ****************************************************************************
// * STRUCTURAL LIMITATION — SINGLE-TRANSITION STRATEGY (red -> FIRST green)   *
// ****************************************************************************
//   The fixed 20/40 bracket is geometrically valid ONLY at the red -> FIRST
//   green flip: the entry bar's PRECEDING brick must be RED. There, under the
//   2x-reversal rule, the reversal direction (2x = 40 = your TARGET) points UP
//   and the continuation direction (1x = 20 = your STOP) points DOWN -> a clean
//   33.3%-breakeven trade that ONE brick resolves.
//
//   You therefore CANNOT use a filter that fires INSIDE a green run (entering
//   AFTER a green brick, chasing a continuation green). There, UP is the
//   continuation (1x = 20) and DOWN is the reversal (2x = 40), so the matching
//   geometry is the MIRROR: stop 40 / target 20, breakeven 66.7%. Running the
//   20/40 bracket on such an entry BREAKS the "brick color == trade outcome"
//   property, and the filter's measured win rate stops matching the real trade.
//
//   RULE: every usable filter must fire at a red->first-green flip. The default
//   F1="1000" does exactly that (green-red-red-red -> fire at the 3rd red's
//   close -> chase the first green). A continuation-entry idea needs a SEPARATE
//   40/20 variant, not this one.
//
//   The SHORT mirror (Scalper_Renko_TransitSHORTrepeat_Layer2) has the symmetric
//   limitation: it fits ONLY the green -> first red flip.
//
// SETUP IN PLAIN ENGLISH:
//   F1 = "1000" in THIS encoding = green(1), red(0), red(0), red(0)
//      = a green bar then 3 red bars = a 3-brick DOWN move.
//   Enter LONG at the close of the 3rd red brick, betting the down-run reverts up.
//   From the bottom of that red brick (2x-reversal rule):
//     next brick RED   (bit 0) = price -20 first  = STOP   = loss
//     next brick GREEN (bit 1) = price +40 first  = TARGET = win
//   F2 = "000" = arm only after 3 such attempts have FAILED (3 losses in a row).
//
// PIPELINE (same as Python trade_filter.py):
//   Every closed Renko bar:
//     1. Determine bit from bar direction (Green/up=1, Red/down=0)
...
//     6. If nextIsMoney -> enter REAL LONG this bar with bracket
//
// REAL TRADE BRACKET (when fired):
//   EnterLong at market (or limit)
//   StopLoss     = entry - StopLossPoints     (price down = bad)  = 1x brick
//   ProfitTarget = entry + ProfitTargetPoints (price up = good)   = 2x brick
//
...
{
    public class Scalper_Renko_TransitLONGrepeat_Layer2 : Strategy
    {
        // ── strategy lifecycle ────────────────────────────────────────────────
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "Renko-based LONG Layer-2 strategy. Each Renko bar = one bit. "
                            + "Green=1 (up), Red=0 (down). Chases '1's (green bars). "
                            + "Stop/Target must equal 1x/2x the data-series brick size. "
                            + "Reconnect-survival via own log file.";
                Name        = "Scalper_Renko_TransitLONGrepeat_Layer2";
                ...
                LogFolder            = @"C:\temp";
                LogBaseName          = "scalper_Renko_TransitLONGrepeat_Layer2";
        ...
        protected override void OnBarUpdate()
        {
            ...
            bool hasOpenPosition = (Position.MarketPosition == MarketPosition.Long);
            ...
            //   Close rose  = brick built UP   = green = bit '1' (WIN for LONG)
            //   Close fell  = brick built DOWN = red   = bit '0' (loss for LONG)
            int bit;
            if (Close[0] > Close[1])
                bit = 1;   // up brick (green), good for long
            else if (Close[0] < Close[1])
                bit = 0;   // down brick (red), bad for long
            else
            {
            ...
            //   side = FAKE_Long     -> ordinary observation brick
            //   win_loss_bit column   -> the RAW brick bit (0=up/green, 1=down/red)
            // Real orders write their own supplementary rows (Long_ENTRY at entry,
            // Long at close, OBS_* if a guard suppresses) — those carry the fill
            // prices and the real outcome bit. Only side=="Long" close rows are read
            // back as real-trade outcomes.
            string barSide = nextIsMoney ? "WOULDBE_TRADE" : "FAKE_Long";
            ...
            bool trendGateOK = !EnableTrendGate || (trendNet >= -TrendGateMaxNetDown);
            ...
            else if (nextIsMoney && EnableRealOrder && !hasOpenPosition && !trendGateOK)
            {
                DiagLog(string.Format(
                    "[TREND GATE] real order suppressed: net{0}={1} beyond -{2} (strong DOWN-run). Pipeline intact.",
                    TrendGateBarsBack, trendNet, TrendGateMaxNetDown));
        ...
        private void TryOpenRealTrade()
        {
            ...
            try
            {
                ...
                {
                    workingEntryOrder = EnterLongLimit(0, true, currentQty, entryPrice, ENTRY_SIGNAL);
                    DiagLog(string.Format(
                ...
                WriteLogRowObs(entryPrice, "Long_ENTRY", currentQty);
            }
            catch (Exception ex)
            {
        ...
        private void StartupDecideAndLoad()
        {
            ...
            DiagLog(Name + " ready (Renko LONG). EnableRealOrder=" + EnableRealOrder
        ...
        private void WriteLogRow(double entryPrice, double exitPrice, double pnl, int bit, int qty, DateTime time)
        {
            try
            {
                string row = string.Format(System.Globalization.CultureInfo.InvariantCulture,
                    ...
                    DateTime.Now, barCount, "Long", qty, entryPrice, exitPrice, pnl, bit,
        ...
        private void MarginFlatten()
        {
            try
            {
                ...
                // from the 1-min clock (BarsInProgress==1). LONG book -> ExitLong only.
                if (Position.MarketPosition == MarketPosition.Long)
                    ExitLong(0, Math.Abs(Position.Quantity), "LR_MarginFlat", ENTRY_SIGNAL);
            }
        ...
        [Display(Name = "Enable Trend Gate", Order = 1, GroupName = "9. Trend Gate",
            Description = "ON by default. Skips a REAL entry when the last N bricks are a strong down-run "
        ...
        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "Trend Gate: max net DOWN-bricks to allow LONG", Order = 3, GroupName = "9. Trend Gate",
            Description = "LONG: skip the entry when net down-bricks over the window EXCEEDS this (a strong selloff). Default 3 (mirror of the SHORT gate).")]
        public int TrendGateMaxNetDown { get; set; }
