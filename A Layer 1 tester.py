def A_layer2_tester_with_full_output(string, F1, F2):
    def make_matcher(pattern):
        return lambda s: s.endswith(pattern)
    
    m1 = make_matcher(F1)
    
    rawString = ""
    isArmed = False
    pending = False
    
    trade_positions = []
    outcome_string = ""
    
    for i, b in enumerate(string):
        # 1. Capture the trade result if pending was set previously
        if pending:
            outcome_string += b
            trade_positions.append({"index": i, "result": b})
            pending = False
            isArmed = False # Reset gate after trade
            
        rawString += b
        
        # 2. Check for trigger (F1) while Armed
        if isArmed and m1(rawString):
            pending = True
        
        # 3. Check for Arming condition (F2)
        if b == F2:
            isArmed = True
            
    return trade_positions, outcome_string

# --- Test ---
s = ""
positions, outcomes = A_layer2_tester_with_full_output(s, F1="101010", F2="0")

print(f"Outcome string: {outcomes}")
print(f"Total trades captured: {len(positions)}")
print(f"Percentage of 1's in outcomes: {outcomes.count('1') / len(outcomes) * 100:.2f}%")
print(f"Trade positions: {positions}")
