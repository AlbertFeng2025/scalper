...
This table is the *pattern-matching* view. **It is not what actually trades** — the
real number of orders is lower because each trade occupies the next brick; see 6.2.
`1000` alone matches once per run (only at length 3).

### 6.2 ⚠️ Market entry occupies the next brick — only *every other* pattern trades

The 6.1 table shows which patterns *match*. What actually *trades* is fewer, and this
is the single biggest source of confusion with multi-pattern firing — so read it
carefully.

Entries are **market orders** (`UseMarketEntry = true`), so an entry fills at the
**next brick's open** and the trade occupies that whole brick — its 20 pt stop sits
exactly on that brick's close (a 20 pt stop on a 20 pt brick). The strategy is
strictly **one trade at a time**: `TryOpenRealTrade` refuses a new entry while any
position *or* working order exists on the instrument. Put together, on a continuing
run:

- `10` fires at green 1's close; its order fills at green 2's open and occupies green 2.
- `100` matches at green 2's close — but the `10` trade is still open there, so `100`
  is **blocked** (logged `WOULDBE_TRADE` / `OBS_ACCOUNT_BUSY`), no order.
- By green 3's close the `10` trade has closed, so `1000` **fires**.

So a continuing run trades **every other** pattern — `10` and `1000`, skipping `100`
— because each one-brick trade eats the brick where the next pattern would fire. You
physically cannot fire two one-brick trades on consecutive bricks.

**Consequences:**

- A long continuing run realistically costs about **2** losses (`10`, `1000`), not the
  3 implied by 6.1. Your downside is *smaller* than the naive count.
- Which pattern gets skipped is a timing artifact, not your choice. If you want the
  list to match what actually trades, use **`10,1000`** — it fires exactly those two,
  nothing wasted. *(In Layer 2 this is different: the skipped `100` still helps arm F2,
  so do NOT trim it there — see the Layer 2 guide.)*
- This is expected, correct one-at-a-time behavior, **not a bug.** Trading all three
  would require holding overlapping positions 20 pt apart (pyramiding), which adds
  exposure and is not recommended for a reversion gamble.
