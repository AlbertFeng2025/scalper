...
**Date of summary:** 2026-07-29 **v2**  *(supersedes 2026-07-29 / 07-28 / 07-25; new material in §14 — a brick-size robustness test that DOWNGRADES the `100/0100` lead)*
...
> - **‼ v2 CONFIDENCE DOWNGRADE — brick size is a load-bearing parameter, and the `100/0100` lead does NOT survive it (§14).** Re-slicing at **80-tick** (20-pt bricks, same 33.3% breakeven) collapses SHORT `100/0100` from 42.9% OOS to **0.0%** (0 of 9 fires) and inverts the whole ordering — at 80-tick the `F1=10` family leads instead. A real structural edge should degrade gracefully with brick size, not flip to zero, so **`100/0100`'s 40-tick result now reads as partly an artifact of the 40-tick brick grid, not a proven edge.** Treat **bar/brick size exactly like the fixed slice size** — a first-class regime parameter that must be re-validated, never changed casually. **Do NOT change the running bar size (or the combo) without a full re-test.** Only `10/0100` shows any (weak) cross-brick consistency.
> - **★ Best improvement found — a brick-native trend gate (§13.5).** Suppressing SHORT `100/0100` fires when the last 10 bricks are strongly up (`net10 ≥ 4`) survived a *clean* train/test split (threshold picked on 5 training days only, tested on 13 untouched): WR 42.9% → **48.8%**, MCL 5 → **3**, per-trade +2.86 → **+4.65 pt**, and — a first for this research — the **95% lower bound (34.6%) clears the 33.3% breakeven.** Theory-driven, threshold-insensitive, helps only the trend-sensitive `100` book. *(v2 note: measured at 40-tick only; per §14 it inherits the same brick-size fragility until re-confirmed across sizes.)*
...
**Bottom line (updated 07-29 v2):** the SHORT-book reversal edge is real but **thin, cost-sensitive, and brick-size-fragile**. **`100/0100` is DOWNGRADED** — it led at 40-tick (48.8% OOS with the gate) but **collapsed to 0% at 80-tick (§14)**, so its edge is not proven robust. The **only cross-brick-consistent combo is `10/0100`** (36% OOS @40t, 50% OOS @80t) — weak, but the best robustness on offer; **`1000` (calibrated slicer)** remains the original. LONG did not hold at 80-tick either. **Retire `10/010`** (flips sign by brick size); **never use `1010`**. **No qty ratchet** (IID); size flat, fractional-Kelly, daily $ cap (§13.3). **Three gates now decide everything, none passed:** (1) live-`rawString` calibration, (2) net-of-costs, (3) **brick-size robustness** — and (3) just failed for the former lead. **Do not change the running bar size or combo without a full re-test.**
...
### 13.6 Revised near-term actions  *(v2: see §14 — `100/0100` downgraded, brick-size gate added)*
...
## 14. ‼ Update — 2026-07-29 v2: brick-size robustness (the lead does NOT transfer)

**Test.** Re-sliced all 18 days at **80-tick** bricks (20 pt/brick → stop 20 / target 40, the *same* 33.3% breakeven and same pipeline) and re-ran the SHORT/LONG combos with the identical 13-day out-of-sample split. This is the same lesson as the earlier slice work: **the fixed size (slice or brick) is a first-class regime parameter, not a cosmetic setting.**

**Result (SHORT, OOS 13 days):**

| filter | 40-tick OOS WR | **80-tick OOS WR** | 80t fires |
|---|---|---|---|
| `100/0100` (40-tick lead) | 42.9% | **0.0%** | 9 |
| `100/00100` | 41.9% | **0.0%** | 6 |
| `10/0100` | 36.1% | **50.0%** | 16 |
| `10/010` | 28.8% (retired) | 44.8% | 29 |

LONG at 80-tick also failed (`100/00100` 25%, `100/0100` 29%, only `10/0100` marginal at 40%/+80).

**Reading it honestly:**
- **The ordering inverts.** At 40-tick, `F1=100` won and `F1=10` was weak; at 80-tick, `F1=100` collapses (0 wins in 9–6 fires) and `F1=10` leads. That is not graceful degradation — it is a sign reversal.
- **`100/0100`'s 40-tick edge reads as partly an artifact of the 40-tick brick grid.** A structural "fade the pullback, catch the reversion" edge should survive a brick-size change with reduced strength, not invert to zero. So the earlier promotion of `100/0100` to lead is **downgraded**: it passed a clean train/test split *within* one brick size but fails *across* brick sizes.
- **Only `10/0100` shows any cross-brick consistency** (above breakeven at both sizes), and even its sibling `10/010` flips sign, so the `F1=10` family is not stable either. **Nothing here is brick-size-robust enough to trade on the 40-tick numbers alone.**
- **Caveat (both directions):** 80-tick halves the brick count (~119/day vs ~145), so `F1=100` fires only 6–9 times OOS — 0/9 is thin and partly luck. But "thin *and* inverted to zero" cannot *support* `100/0100`; the sample can't rescue it.

**What changes:**
1. **Downgrade `100/0100`** from "lead candidate" to "40-tick-only, not robust." The trend gate (§13.5) inherits the same caveat (measured at 40-tick).
2. **Elevate `10/0100`** to "least-fragile candidate" — but only weakly; it still needs the other gates.
3. **Add a third validation gate: brick-size robustness.** Alongside live-`rawString` calibration and net-of-costs, a candidate must clear breakeven at **≥2 brick sizes** before it is trusted. The current lead just failed this.
4. **Operational directive: do NOT change the running bar size (or the filter combo) without a full re-test.** Bar size is load-bearing — treat any change as a new strategy requiring the whole OOS + robustness pipeline, exactly as a change in fixed slice size would.

*End of summary — updated 2026-07-29 v2 (§14 added; §12/§13/07-28/07-25 retained above; core research 2026-07-25).*
