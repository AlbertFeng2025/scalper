    ...
    public class Scalper_Shortrepeat_Layer2 : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                // margin cutoff (early EOD before broker overnight-margin snapshot)
                EnableMarginCutoff   = true;
            ...
            try
            {
                ...
                if (wasMoneySlice)
                {
                    ...
                    if (Position.MarketPosition == MarketPosition.Short)
                    {
                        ...
                        awaitingClose = false; entryInFlight = false; workingEntryOrder = null;
                        // MONEY (broker side): the bracket did NOT fill and we force-closed -> count
                        // this abnormal close as a LOSS (0), conservatively, for qty rule / breaker.
                        realTradeOutcome.Append("0");
                        RecordSessionOutcome(0);
                        realLossesInARow++;
                        DiagLog("[REAL LOSS forced-close] realLossesInARow=" + realLossesInARow);
                        // PATTERN (data feed): cross-line bit, its own world.
                        UpdatePipeline(bit);
                    ...
                    {
                        // Money was already recorded by OnExecutionUpdate (the real bracket fill).
                        // Still record the PATTERN bit here from the data feed (cross-line) -- every
                        // slice gets its rawString bit, exactly like a Renko brick records green/red.
            ...
            string orderId, DateTime time)
        {
            ...
            // ONE bit for the whole (possibly split) trade once the position is flat:
            //   Stop loss -> loss contracts;  Profit target -> win contracts;
            //   our flatten (SR_Flatten / SR_ForceClose) -> does NOT vote (forced cleanup);
            //   anything else (e.g. session-close forced exit) -> loss (conservative).
            // The trade still produces exactly ONE bit; contracts only decide which bit. A force-
            // flattened trade (no stop/target fills, or mostly stop) records a LOSS (0), so it counts.
            ...
            // Use the passed-in marketPosition (position AFTER this execution) -- NT docs call this
            // the reliable value during simultaneous fills; it reads Flat only on the TRUE final close,
            // so the ONE bit is recorded once from the complete tally (pattern pipeline stays correct).
            if (marketPosition == MarketPosition.Flat && awaitingClose)
            {
                ...
                // PATTERN NOTE: rawString is written ONLY by CheckFakeSlice from the data feed
                // (cross-line). This broker/money handler must NEVER write the pattern string.
        ...
        private void ProcessShutdown()
        {
            ...
            try
            {
                ...
                // FLIP a short into a long) -- with an EMPTY-signal market exit (not tied to any
                // entry, so NinjaTrader cannot ignore it). Bounded + throttled: never loops/spams.
                if (!flattenGaveUp
        ...
        private void MarginProcess()
        {
            ...
            // cancel our own live orders first (never our own SR_MarginFlat), then close the LIVE
            // position with an EMPTY-signal exit -- both directions, so a flipped position is caught.
            // MarginProcess is re-driven every tick, so it retries naturally until flat (no disable).
            if (Position.MarketPosition != MarketPosition.Flat)
            {
                foreach (Order o in Orders)
                {
