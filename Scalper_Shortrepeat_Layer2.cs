...
// Scalper_Shortrepeat_Layer2
//
// Based on scalper_SHORTrepeat_ReviewIfQtyMorethan1 (Patch 7)
//
// LAYER 2 UPGRADE
// ---------------
// Pipeline:
...
//
// SHORT-specific fake trade resolution (unchanged from Patch 7):
//   fakeEntryPrice  = ask at moment entry fires
//   fakeStopPrice   = fakeEntryPrice + StopLossPoints    (ABOVE entry)
//   fakeTargetPrice = fakeEntryPrice - ProfitTargetPoints (BELOW entry)
//   Stop hit   when ask >= fakeStopPrice
//   Target hit when bid <= fakeTargetPrice
...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class Scalper_Shortrepeat_Layer2 : Strategy
    {
        ...
        private bool   waitingForExpected = false;  // FIRE just happened → next fake = expected outcome
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description  = "SHORT scalper with two-layer pattern filter. "
                ...
                Filter2Pattern       = "01";
                BaseQuantity         = 1;
                MaxTotalTradeCount   = 100;
                MaxRealLossInARow    = 3;
                LogFilePath          = @"C:\temp\Scalper_Shortrepeat_Layer2_log.csv";
            }
            else if (State == State.Configure)
            {
                // SHORT: SetTrailStop ratchets DOWN as price falls
        ...
        // =====================================================================
        // AppendOutcome — called after EVERY fake or real trade outcome.
        // Implements two-flag state machine matching apply_filter logic:
        //
        //   waitingForL1   : F1 matched L0 tail → next raw digit → L1
        //   waitingForTrade: F2 matched L1 tail → next L1 digit → REAL TRADE
        //
        // Flow per digit:
        //   1. Append bit to L0.
        //   2. If waitingForL1:
        //        append bit to L1.
        //        if waitingForTrade → fire REAL TRADE.
        ...
        private void AppendOutcome(int bit)
        {
            ...
            if (waitingForL1)
            {
                ...
                {
                    waitingForTrade   = false;
                    waitingForExpected = true;
                    DiagLog(string.Format(
                        "[FIRE] F2 armed. L1 digit='{0}' → placing REAL SHORT trade. L1={1}",
                        bit, l1));
                    PlaceRealEntry();
                }
        ...
        private void PlaceEntry()
        {
            ...
            ordersPlaced++;
            double refPrice = GetCurrentAsk();   // SHORT sells at ask
            ...
            DiagLog(string.Format(
                "FAKE SHORT ENTRY #{0} @ {1:F2} stop={2:F2}(above) target={3:F2}(below)",
                ordersPlaced, fakeEntryPrice, fakeStopPrice, fakeTargetPrice));
        }

        // =====================================================================
        // PlaceRealEntry — fires a real SHORT order immediately
        ...
        private void PlaceRealEntry()
        {
            ...
            try
            {
                if (UseMarketEntry)
                {
                    workingEntryOrder = EnterShort(qty, ENTRY_SIGNAL);
                    DiagLog(string.Format("REAL SHORT ENTRY #{0} MARKET qty={1}", ordersPlaced, qty));
                }
                else
                {
                    double limitPx = GetCurrentAsk() + LimitOffsetPoints;
                    limitPx = Instrument.MasterInstrument.RoundToTickSize(limitPx);
                    workingEntryOrder = EnterShortLimit(0, true, qty, limitPx, ENTRY_SIGNAL);
                    DiagLog(string.Format("REAL SHORT ENTRY #{0} LIMIT qty={1} limit={2:F2}",
        ...
        // =====================================================================
        // CheckFakeTrade — SHORT-specific resolution
        // Stop hit   when ask >= fakeStopPrice  (price went UP against short)
        // Target hit when bid <= fakeTargetPrice (price went DOWN for short)
        // =====================================================================
        private void CheckFakeTrade()
        {
            try
            {
                ...
                bool stopHit   = ask >= fakeStopPrice;
                bool targetHit = bid <= fakeTargetPrice;
                ...
                DiagLog(string.Format(
                    "FAKE SHORT {0}: entry={1:F2} exit={2:F2} pnl={3:0.00}",
            ...
            string orderId, DateTime time)
        {
            ...
            if (oName == ENTRY_SIGNAL && (isFull || isPart))
            {
                ...
                if (isFull)
                {
                    ...
                    DiagLog(string.Format("Entry complete. Fill={0:F2} qty={1}. Trail={2}.",
                        ...
                        EnableTrailingStop ? "ENABLED (NT SetTrailStop SHORT)" : "disabled (fixed stop)"));
                }
                return;
            }
            ...
            if ((isStopFill || isTargetFill) && isFull)
            {
                ...
                if (Position.MarketPosition == MarketPosition.Flat)
                {
                    ...
                    DiagLog(string.Format(
                        "CLOSED SHORT {0}: entry={1:F2} exit={2:F2} qty={3} pnl={4:0.00} bit={5}",
        ...
        private void ProcessShutdown()
        {
            if (entryInFlight && workingEntryOrder != null)
            {
                    ...
                    || os == OrderState.Submitted)
                {
                    ...
                    {
                        DiagLog(string.Format(
                            "Shutdown: cancelling unfilled SHORT entry order (state={0}).", os));
        ...
        private void WriteLogRowFake(double exitPrice, double pnl, int bit)
        {
            try
            {
                string row = string.Format(
                    System.Globalization.CultureInfo.InvariantCulture,
                    "{0:yyyy-MM-dd HH:mm:ss},{1},{2},{3},{4},{5},{6:0.00},{7},{8},{9}\n",
                    DateTime.Now, ordersPlaced, "FAKE_Short", 0,
        ...
        [Display(Name = "Enable Trailing Stop", Order = 12, GroupName = "Bracket",
            Description = "When true: NT native SetTrailStop — SHORT stop ratchets DOWN as price falls. "
        ...
        [Display(Name = "Filter 2 Pattern (comma-separated)", Order = 3, GroupName = "Filter & Real Order",
            Description = "Pattern checked against Layer 1 tail after every Layer 1 append. "
                        + "Match + Enable Real Order = true → real SHORT trade fires. "
